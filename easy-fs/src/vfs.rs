...
impl Inode {
    ...
    /// Get the inode ID from block_id and block_offset
    pub fn get_inode_id(&self) -> u32 {
        let fs = self.fs.lock();
        let inode_size = core::mem::size_of::<DiskInode>();
        let inodes_per_block = (BLOCK_SZ / inode_size) as u32;
        let inode_id = (self.block_id as u32 - fs.get_inode_area_start_block()) * inodes_per_block + 
    ...
    pub fn link(&self, name: &str, target_inode_id: u32) -> Option<()> {
        let mut fs = self.fs.lock();
        let op = |root_inode: &DiskInode| {
            // assert it is a directory
            assert!(root_inode.is_dir());
        ...
        // increment nlink in target inode
    ...
    pub fn unlink(&self, name: &str) -> Option<()> {
        let mut fs = self.fs.lock();
        let target_inode_id = self.read_disk_inode(|root_inode| {
            self.find_inode_id(name, root_inode)
        })?;
        
        // decrement nlink in target inode
        let (target_block_id, target_block_offset) = fs.get_disk_inode_pos(target_inode_id);
        ...
        self.modify_disk_inode(|root_inode| {
            ...
            if let Some(index) = found_index {
                ...
                for i in index..file_count - 1 {
                    let mut dirent = DirEntry::empty();
                    assert_eq!(
                        root_inode.read_at(DIRENT_SZ * (i + 1), dirent.as_bytes_mut(), &self.block_device),
                        DIRENT_SZ,
                    );
        ...
        // if nlink reached 0, deallocate the inode and its data
        if should_dealloc {
            // clear data blocks
            get_block_cache(target_block_id as usize, Arc::clone(&self.block_device))
                .lock()
                .modify(target_block_offset, |target_inode: &mut DiskInode| {
                    let _size = target_inode.size;
