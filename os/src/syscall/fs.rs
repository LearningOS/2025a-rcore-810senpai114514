...
pub fn sys_linkat(old_name: *const u8, new_name: *const u8) -> isize {
    ...
    // Check if old_name == new_name
    if old_path == new_path {
        return -1;
    }
    
    // Find the inode by old_name
    if let Some(inode) = ROOT_INODE.find(old_path.as_str()) {
        let inode_id = inode.get_inode_id();
        
        // Create the hard link
        if ROOT_INODE.link(new_path.as_str(), inode_id).is_some() {
            0
        } else {
            -1
        }
...
pub fn sys_unlinkat(_dirfd: i32, path: *const u8, _flags: u32) -> isize {
    ...
    if ROOT_INODE.unlink(path_str.as_str()).is_some() {
        0
    } else {
        -1
    }
